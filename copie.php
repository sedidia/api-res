...
// les fonctions
// PAIEMENT EN COURS
function paiementEnCours($promotion,$success,$msg,$paiement,$id_jury){
    global $bdd;
...
// EN CAS D'ERREUR
function ResponsePaiements($success,$msg){
    $res = array(
        'success' => $success,
        'msg' => $msg
    );
                ...
                if($etat_j == "0"){                    
                    foreach ($dataReques as $paiement) {
                        ...
                        if($nom_du_jury == $nom_du_jury_grille){
                            ...
                            paiementEnCours($promotion,$success,$msg,$paiement,$id_jury);
                            $appel = $appel + 1;
                            if($appel == 1){
                                echo paiementEnCours($promotion,$success,$msg,$paiement,$id_jury);
                            }
                        }else{
                            $success = 0;
                            $msg = "Erreur : vous n'etes pas autorisé d'enregistrer les paiements de la faculté des ".$nom_faculte;
                            ResponsePaiements($success,$msg);
                            $appel = $appel + 1;
                            if($appel == 1){
                                echo ResponsePaiements($success,$msg);
                            }
                        }                    
                    }
                }else{
